...
parser <- add_argument(parser = parser,
                       arg = c("input", "--whatOutID", "--remResDups", "--symbsInCols", "--saveIDsDF"),
                       help = c("Input file with either colnames or rownames in gene symbols",
                                ...
                                "If IDs_DF, showing equivalences between symbols, ENTREZID and ENSEMBL, should be exported."),
                       flag = c(F, F, T, T, T))
...
if(length(NA_symbs) > 0){
        # If the number of NA genes is higher than 100 split it in multiple chunks
        # to be able to download them
        if(length(NA_symbs) < 100){
                genesNA_idMapped <- symb_to_entrez_ncbi(NA_symbs)
        }else{
                nGroups <- floor(length(NA_symbs)/100)
                genesNA_idMapped <- data.frame(matrix(nrow = 0,
                                                      ncol = 3,
                                                      dimnames = list(NULL,
                                                                      c("symbol",
                                                                        "newSymbol",
                ...
                for(i in 1:nGroups){
                        ...
                        NA_symbs_subset <- NA_symbs[start:end]
                        toBind <- symb_to_entrez_ncbi(NA_symbs_subset)
                        genesNA_idMapped <- rbind.data.frame(genesNA_idMapped,
                                                             toBind)
                }
                print(head(genesNA_idMapped))
                if(end < length(NA_symbs)){
                        toBind <- symb_to_entrez_ncbi(NA_symbs[(end + 1):length(NA_symbs)])
                        genesNA_idMapped <- rbind.data.frame(genesNA_idMapped,
                                                             toBind)
                }
                print(head(genesNA_idMapped))
...
# Export IDs_DF if indicated by user
if(saveIDsDF){
        IDs_DF_outName <- sprintf("%s%s_IDsDF.csv",
                                  outDir,
                                  name4Outs)
        write.csv(IDs_DF, file = IDs_DF_outName)
}
